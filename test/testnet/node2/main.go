...
var disable = flag.Bool("disable", true, "disable transfer test")
...
func main() {
	...
	err = channelP2p.Start(tc.Parameters.BaseConfig.Init2ListenAddr)
	if err != nil {
		log.Fatal(err)
		return
	}
...
func currentBalance(channel *ch.Channel) {
	...
	for {
		chanState := channel.Service.GetChannel(registryAddress, tokenAddress, tc.Dns1Addr)
		if chanState != nil {
